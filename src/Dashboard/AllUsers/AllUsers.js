...
const AllUsers = () => {
    ...
    const handleDeleteUser = user => {
        fetch(`https://skypearl-it-server.vercel.app/users/${user?._id}`, {
    ...
    const { data: users = [], refetch } = useQuery({
        queryKey: ['users'],
        queryFn: async () => {
            const res = await fetch('https://skypearl-it-server.vercel.app/users');
    ...
    const handleMakeAdmin = id => {
        fetch(`https://skypearl-it-server.vercel.app/users/admin/${id}`, {
            method: 'PUT',
            headers: {
                authorization: `bearer ${localStorage.getItem('accessToken')}`
            }
